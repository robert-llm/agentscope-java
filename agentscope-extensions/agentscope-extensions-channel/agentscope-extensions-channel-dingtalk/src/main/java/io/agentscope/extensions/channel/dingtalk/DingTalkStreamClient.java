 ...
 * </ol>
 *
 * <p>This client is intentionally small — no external SDK dependency, no schema validation beyond
 * what's required. Heartbeat / ping is left to the JDK WebSocket implementation.
 */
public final class DingTalkStreamClient {
    ...
    private Endpoint openConnection() throws Exception {
        ...
        body.put("ua", "agentscope-builder/1.0");
        body.set(
                "subscriptions",
                MAPPER.valueToTree(
    ...
    private void handleFrame(String frameText) {
        ...
        if (!"CALLBACK".equalsIgnoreCase(type)) {
            // Ignore SYSTEM/EVENT frames in MVP.
            return;
        }

        // Send ACK before dispatching so the gateway releases the message; the user-visible reply
        // will be sent separately via DingTalkOutboundClient.
        ackFrame(envelope);
